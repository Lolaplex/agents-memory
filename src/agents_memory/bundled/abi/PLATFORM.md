...
The Python reference ships `python -m agents_memory ingest` with machine-specific paths (your Agent transcript folders, VS Code, OpenAI export zip). On another machine, edit those paths or rely on MCP `add_memory` after manual distill. Ingest is **not** part of the core ABI — only the resulting `chats-index.md` shape is.
